...
def ellipse_points(r,x,y,z,res,shrink):
    ...
    for i in range(res+1):
        ...
        zintp = math.sqrt( (1/shrink) *((r**2) - ((yels[i]-y)**2)) )  + lipz
        zintn = ((-1) * math.sqrt( (1/shrink) *((r**2) - ((yels[i]-y)**2)) ) ) + lipz
        zpos.append(zintp)
        zneg.append(zintn)
...
def engine(cg,r,length,cg_shift,shrink,ax):
    ...
    print('\nyes is: ',yes)
    print('\nzes is: ',zes)
    return (ax,yes,zes,zpos,cg,length,cg_shift,r,shrink,lipz)
...
#-----------------------z bumping section------------------------#   
    for k in range(len(ygc)):
        ...
        if k in over:             # UP bump for OVER gc's
            ...
            frac = transcub.frac_finder(exam,cub_behind)
            ...
            #### File through a second time to perform the appropriate adjustments
            for l in range(len(ygc[k])):
                if (countbeh <= gcnumbeh) and (k >= (over[-(gcnumbeh+1)])):
                    zog = frac * zgc[k][l]
            ...
            #### Preliminary calcs for longitudinal smoothing
            index = index_finder(ygc[k],cg[1])
            exam = ( xgc[k][index] - (cg[0]+shift) ) /z0f
            frac = transcub.frac_finder(exam,cub_front)
...
def main(xgc,ygc,zgc,vals):
    ...
    cg_shift = vals["ducted fan cg shift [in]"]
    t = vals["ducted fan cover thickness [in]"]
    shrink = vals["ducted fan exhaust shrink factor [in]"]
    ...
    (yes,zes,zpos,lipz) = engine(cg,r,length,cg_shift,shrink,ax)

    (xnewgc,ynewgc,znewgc) = zgc_bumpNadd(xgc,ygc,zgc,yes,zes,zpos,cg,length,cg_shift,r,shrink,lipz,t,srB,srO,srF,gcnumbeh,gcnumfro,ax)
